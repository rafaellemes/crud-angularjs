App.controller('MedicoIACtrl', ['$scope', '$base64',
  'MedicoService', 'Util', '$window', 'TelefoneService', 'EnderecoService',
  function($scope, $base64, MedicoService, Util, $window, TelefoneService, EnderecoService) {
    ...
    function init() {
      ...
      var medico = sessionStorage.medico;
      console.log("init");
        console.log(medico);

      //var medico = {};
      if (medico && medico != "undefined") {
        var jFunc = JSON.parse($base64.decode(medico));
        console.log(Util.copyJson(jFunc));
        $scope.medico.dados = Util.copyJson(jFunc);
        alterar = true;
        $scope.nomePagina = "Alterar";
        $scope.icone = "fa-edit"

        listarTelefonePessoa($scope.medico.dados.id);
      }
    ...
    $scope.voltar = function(){
      sessionStorage.medico = undefined;
        $window.location.href = '#/admin/medicos';
    }

    $scope.salvar = function() {
      $scope.showErrorMessage = false;
      $scope.showSuccessMessage = false;
      var form = $scope.medico;
      console.log("Médico");
      console.log(form);
      ...
      if (alterar) {
        console.log("alterar");
        alterarMedico(Util.copyJson(form));
      } else {
        console.log("salvar");
        salvarMedico(form);
      }
    ...
    // telefones
      ...
      function cadastrarTelefone(telefone)
      {
        ...
        TelefoneService.salvar(telefone).success(function(result){
          ...
          tel_pessoa.dados.id_pessoa = $scope.medico.dados.id;
    ...
    function listarTelefonePessoa(id_pessoa){
      ...
      .success(function(result){
        ...
        listarEnderecoPessoa($scope.medico.dados.id);
      })
      .error(function(data, status, headers, config){
        //$scope.msgError = "ERRO: Houve problemas ao salvar/alterar o telefone!";
        //$scope.showTelErrorMessage = true;
    ...
    function cadastrarEnderecoPessoa(id_endereco){
      ...
      insert.dados.id_pessoa = $scope.medico.dados.id;
    ...
    function salvarMedico(dados) {
      ...
      MedicoService.salvar(dados).success(function(result) {
        ...
        $scope.medico.dados.id = result.insertId;
        console.log(result);
